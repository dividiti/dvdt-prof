...
with open(call + _id + '.cpp', 'r') as f:
    ...
    source['num_kernels_ret_ptr'] = re.search('\(cl_uint(\s*)\*\)(\s*)(?P<num_kernels_ret_ptr>%s)' % ptr_regex, source['text']).group('num_kernels_ret_ptr')
...
status &= (source['num_kernels_ret_ptr'] == result['num_kernels_ret_ptr'])
status &= (0 == result['num_kernels_ret'])
